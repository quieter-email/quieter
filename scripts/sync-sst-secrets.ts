...
import {
  githubSstOptionalSecrets,
  githubSstSecrets,
} from "@quieter/env/github";
...
const hasConfiguredSecret = (environmentName: string) =>
...
const configuredSecrets = Object.entries(syncableSecrets).filter(
  ([environmentName]) => hasConfiguredSecret(environmentName)
);
const missing = Object.keys(githubSstSecrets).filter(
  (name) => !hasConfiguredSecret(name)
);
...
const secretsFile = path.join(
  tmpdir(),
  `quieter-sst-secrets-${crypto.randomUUID()}.env`
);
const contents = configuredSecrets
  .map(
    ([environmentName, sstName]) =>
      `${sstName}=${JSON.stringify(process.env[environmentName])}`
  )
  .join("\n");
...
try {
  ...
  if (exitCode !== 0) {
    throw new Error(
      `SST secret synchronization failed with exit code ${exitCode}.`
    );
  }
} finally {
