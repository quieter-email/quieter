...
const [ownerEmail, mode = "seed"] = process.argv.slice(2);
if (
  ownerEmail === undefined ||
  !ownerEmail.includes("@") ||
  !["seed", "incoming"].includes(mode)
) {
  throw new Error(
    "Use dev:fixtures <email-of-your-local-login> [seed|incoming]."
  );
}
const response = await fetch("http://127.0.0.1:8787/__dev/mail/seed", {
  body: JSON.stringify({ incoming: mode === "incoming", ownerEmail }),
  headers: {
