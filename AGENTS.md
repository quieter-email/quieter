...
- Domain setup uses signed Domain Connect templates when the discovered DNS provider confirms support for Quieter's exact service template. Every callback must still run Quieter's DNS verification; unsupported providers and incomplete changes use the same manual DNS record view.
