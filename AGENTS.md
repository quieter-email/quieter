...
- Pull request review deployments are opt-in through `.github/workflows/review-deploy.yml` and the GitHub `Review` environment. The workflow resolves a same-repository pull request to an exact SHA, verifies it without secrets, and promotes it manually to the fixed `https://review.quieter.email` SST stage. It is never a required pull request check and must not block or delay merging. Fork pull requests cannot be promoted, `pull_request_target` must not execute pull-request code, and dynamic `pr-*` stages must not be reintroduced. Review deployments use isolated credentials and a synthetic non-production database, do not run remote database migrations, and admit first-time Google sign-in only through the Review OAuth project's explicit test-user list. Shared Review deployments keep preview personas disabled; personas remain a local-only testing aid.
- The GitHub production environment must also provide `POLAR_PRODUCT_MANAGED_ID`, `POLAR_PRODUCT_PRO_ID`, and the current operational `AI_USD_TO_EUR_RATE`.
