...
- Gmail useful details are a separate explicit per-mailbox Pro opt-in. Newly added Inbox messages are classified with `openai/gpt-5-nano` into a constrained taxonomy: short-lived verification codes and delivery updates. Verification codes are encrypted at rest, expire ten minutes after receipt, and are removed by regular reconciliation. Delivery updates deduplicate by carrier and tracking number when available, remain until their controlled expiry, and never rely on model-selected retention periods. Tokens report through the user AI usage meter, and persisted per-message events make retries idempotent.
