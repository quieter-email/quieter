...
- Staging deploys run through `.github/workflows/staging-deploy.yml` from the `staging` branch, using the GitHub `staging` environment. Staging runs quality checks, applies committed migrations to a separate staging database with `QUIETER_ALLOW_REMOTE_MIGRATIONS=staging`, pulls Vercel Preview environment variables scoped to the `staging` branch, then builds and uploads a Vercel Preview deployment with the Vercel CLI. Staging must not share the production database and does not deploy SST/AWS/Cloudflare mail infrastructure unless a separate staging stack is intentionally added later.
- Pull request Vercel previews run through `.github/workflows/vercel-pr-preview.yml` only for same-repository PR branches using the GitHub `preview` environment. The workflow pulls Vercel Preview variables scoped to the `staging` branch so PR previews share staging's isolated non-production service configuration without exposing secrets to forks. Fork PRs must not receive Vercel tokens or preview environment secrets. Do not use `pull_request_target` to build or deploy PR code. Vercel Preview variables must be non-production only; branch-specific Preview variables are allowed for trusted long-lived branches. Gmail AI automation stays disabled outside Vercel production unless `QUIETER_GMAIL_AI_AUTOMATION_ENABLED=true` is set for a controlled test environment with isolated keys and hard provider caps.
