...
- DB: Drizzle + Postgres (postgres-js against loopback Postgres or PlanetScale; Cloudflare Workers use Hyperdrive). PlanetScale production context is organization `leander-riefel`, database `quieter`, branch `main`, engine PostgreSQL 18. The branch contains isolated logical databases `quieter` for production and `quieter_dev` for development, with separate app and migrator roles and cross-database access revoked. Read-only inventory is allowed; database writes, role or credential changes, backups, and production operations require explicit approval. Every Worker request, including local Cloudflare Vite development, owns its Postgres client through `withRequestDatabaseClient`; never reuse an I/O-bound client across requests.
...
- Gmail AI auto-labeling is an explicit per-mailbox opt-in. It processes only newly added Inbox messages and applies existing custom Gmail labels; only labels with persisted inclusion criteria participate, and the model must satisfy those criteria with direct evidence. It never creates labels. The user-level cloud model preference defaults to `openai/gpt-5.6-luna`. Dynamically relevant mailbox knowledge informs classification without replacing current-message evidence; background classification never borrows a billing user's personal context. Tokens report through the user AI usage meter, and persisted per-message decisions make queue retries idempotent.
- Gmail useful details are a separate explicit per-mailbox Pro opt-in. Newly added Inbox messages are classified with the user-level cloud model preference, which defaults to `openai/gpt-5.6-luna`, against a fixed conservative taxonomy and dynamically relevant knowledge consolidated from explicit useful/not-useful feedback. Sender-domain feedback applies immediately, while mailbox-wide category suppression requires repeated feedback; avoided categories are rejected both in the prompt and after classification. Only high-confidence results are accepted. The model selects relevance windows within server safety caps; future items remain hidden until relevant. Verification codes are encrypted at rest. Users can rate or dismiss one item, and disabling the feature deletes every item immediately while retaining preference feedback. Tokens report through the user AI usage meter, and persisted per-message events make retries idempotent.
- Dynamic AI knowledge has two scopes: private personal knowledge that follows a user, and mailbox knowledge that is more specific for the current mailbox. `aiMemory` stores atomic user-authored instructions and learned records; mailbox instructions override personal instructions, and instructions override contradictory learned records. Every contextual behavior agent must retrieve through `loadAiAgentContext` rather than loading a profile blob or querying memory tables directly. Interactive agents may use both accessible scopes; background agents with no acting user use mailbox knowledge only. Provider type, mailbox ownership, and whether a mailbox is private or shared never change memory behavior. Every user-authored send can update both the acting user's personal profile and the current mailbox profile. Settings exposes a scoped natural-language knowledge conversation, auditable/undoable changes, forget/delete/export controls, and a per-scope active-learning prompt. High-signal actions persist compact observations for consolidation, while current or exhaustive questions use live mailbox tools instead of turning mail history into memory. See `docs/ai-memory.md`.
- The default chat model is device-local; selecting a model in chat updates that default, while chats with messages reopen on their most recently used model. Auto-labeling and useful-details model preferences remain user-level cloud settings.
...
- Mail domains have an explicit `send_only` or `send_and_receive` mode. Only verified receive-enabled domains can create managed mailboxes or accept inbound mail, and switching a domain to send-only is blocked while managed mailboxes exist.
- Domain setup uses signed Domain Connect templates when `DOMAIN_CONNECT_PRIVATE_KEY_B64` is configured and the discovered DNS provider confirms support for Quieter's exact service template. That signing key is optional until Domain Connect provider registration is ready (`githubSstOptionalSecrets`); without it, setup stays on the manual DNS record view. Every callback must still run Quieter's DNS verification.
...
- Production deploys run through `.github/workflows/sst-deploy.yml` on pushes to `main` or manual dispatch, using the protected GitHub `production` environment. The deploy calls `.github/workflows/ci-main.yml` as the shared reusable quality and migration-integration gate, then applies committed forward migrations, synchronizes GitHub secrets into SST's encrypted secret store, and deploys both the AWS mail/background stack and Cloudflare web Worker through SST. Generated SST outputs are bound directly to the Worker; do not copy them into another dashboard. GitHub is the only manually configured source of deployment variables and secrets. The required runtime secret map lives in `packages/env/src/github.ts`, and `scripts/sync-sst-secrets.ts` publishes it before every deploy. Remote migration credentials remain protected GitHub-only values, automated production migrations reject destructive SQL, and production migration history is never adopted or rewritten automatically.
- Pull request review deployments are opt-in through `.github/workflows/review-deploy.yml` and the GitHub `Review` environment. Only the repository owner, validated by exact GitHub login and immutable user id, can trigger one by posting the `quieter review` command on an open same-repository pull request or by using the owner-only manual fallback. Surrounding whitespace is ignored, but no other command text is accepted. The comment is the durable authorization signal: accepted commands receive a rocket reaction, later pushes redeploy automatically, and every promoted revision reports progress through the non-required `Review deployment` check. The workflow resolves the pull request to an exact SHA, builds and bundles it without secrets, then uses trusted default-branch configuration to upload the inert artifact to the fixed `https://review.quieter.email` Worker. Normal pull request CI owns checks and tests so Review deployment stays focused on producing the preview artifact. The trusted deploy job prepares the synthetic Review database for the pull request (wipe on first Review deploy for that PR, migrate-only on later pushes), applies the promoted revision's committed expand migrations with a Review-only `DATABASE_MIGRATION_URL`, grants the Review app role, then the preview-only `/api/review-health` probe validates the expected schema before the deployment reports success. Pull-request package scripts, build tools, and configuration never receive the Cloudflare token; migration SQL comes from the authorized revision while the prepare/migrate runner stays on trusted default-branch tooling. The workflow is never a required pull request check and must not block or delay merging. Fork pull requests cannot be promoted, `pull_request_target` must not execute pull-request code, and dynamic `pr-*` stages or provider preview URLs must not be reintroduced. Review runtime credentials and the synthetic database are isolated from production, and first-time Google sign-in is limited to the Review OAuth project's explicit test-user list. Use only dedicated test mailboxes for Gmail authorization. Shared Review deployments keep preview personas disabled; personas remain a local-only testing aid.
- The GitHub production environment must also provide `POLAR_PRODUCT_MANAGED_ID` and `POLAR_PRODUCT_PRO_ID`.
- Local development uses loopback Postgres or the isolated `quieter_dev` logical database in the shared PlanetScale cluster. Its exact host is pinned with `QUIETER_LOCAL_PLANETSCALE_HOST`; `DATABASE_URL` must use the dev app role through port 6432 and `DATABASE_MIGRATION_URL` must use the dev migrator role through direct port 5432. Arbitrary remote and production logical database URLs remain rejected. `bun run dev` directly starts Vite's local Worker as the only foreground process, and Vite validates the database allowlist before serving. Database migrations are explicit through `vp run db:migrate`, not a side effect of starting the server. Chat generation, mail automation, and mailbox actions use in-process fallbacks so Ctrl+C stops the complete local runtime without a custom process orchestrator. The local Worker config and `.env.local` live at the workspace root so the Cloudflare Vite plugin loads local bindings directly without a command-line env wrapper. `vp run env:doctor` must pass after `.env.local` edits; it rejects persistent cloud queue/live-sync endpoints, production-shaped service keys, non-sandbox Polar credentials, and unallowlisted databases. CI migration verification uses its temporary Postgres service container; do not provision hosted databases for pull requests. Remote SST mail/background infrastructure is explicit through `bun run dev:mail` or `bun run dev:cloud`, can outlive the terminal, and uses non-production credentials from `.env.sst.local`. Direct SST package commands load those env files without a process wrapper; pass `--config sst.config.ts --stage mail-dev` to generic `bun run sst -- ...` commands.
- Vite Task derives production build order from package dependencies. Keep custom task inputs and environment variables explicit whenever enabling task caching. Development resolves private workspace packages through their `development` source exports, and Node-loaded Vite configuration must depend only on source-loadable modules so `vp run dev` never requires package builds first.
