...
- Consent backend: `apps/web/src/lib/c15t.server.ts` + `apps/web/src/routes/api/c15t.$.ts`. Kysely adapter on Neon with `c15t_` table prefix. Production consent migrations run only in `.github/workflows/sst-deploy.yml` via `bun run consent:migrate`; runtime requests never run DDL. Exempt `/api/c15t` and legal routes from the site-password gate.
...
- Gmail useful details are a separate explicit per-mailbox Pro opt-in. Newly added Inbox messages are classified with `openai/gpt-5-nano` against a fixed conservative taxonomy, and only high-confidence results are accepted. The model selects relevance windows within server safety caps; future items remain hidden until relevant. Verification codes are encrypted at rest. Users can dismiss one item or disable the feature to delete every item immediately. Tokens report through the user AI usage meter, and persisted per-message events make retries idempotent.
