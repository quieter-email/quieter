...
- Local development uses loopback Postgres or a dedicated disposable Neon branch whose exact direct endpoint is pinned with `QUIETER_LOCAL_NEON_HOST`; arbitrary remote and production database URLs remain rejected. `bun run dev` directly starts Vite's local Worker as the only foreground process, and Vite validates the database allowlist before serving. Database migrations are explicit through `vp run db:migrate`, not a side effect of starting the server. Chat generation, mail automation, and mailbox actions use in-process fallbacks so Ctrl+C stops the complete local runtime without a custom process orchestrator. The local Cloudflare Vite config dynamically allowlists only variables present in `.env.local`. `vp run env:doctor` must pass after `.env.local` edits; it rejects persistent cloud queue/live-sync endpoints, production-shaped service keys, non-sandbox Polar credentials, and unallowlisted databases. CI migration verification uses its temporary Postgres service container; do not provision hosted databases for pull requests. Remote SST mail/background infrastructure is explicit through `bun run dev:mail` or `bun run dev:cloud`, can outlive the terminal, and uses non-production credentials from `.env.sst.local`. Direct SST package commands load those env files without a process wrapper; pass `--config sst.config.ts --stage mail-dev` to generic `bun run sst -- ...` commands.
