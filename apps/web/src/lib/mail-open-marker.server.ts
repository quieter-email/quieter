...
export const handleMailOpenMarker = async (
  token: string
): Promise<Response> => {
  const secret = serverEnv.BETTER_AUTH_SECRET;
  const messageHeaderId =
    token !== "" && secret !== undefined && secret !== ""
      ? verifyOpenTrackingToken(token, secret)
      : null;
  ...
  try {
    const rateLimit = await consumeRateLimit({
      key: `open-marker:${createHash("sha256").update(token).digest("hex")}`,
      limit: 60,
      windowMs: 60_000,
    });
