...
const readDemoModeEnabled = () => {
  if (!isDemoModeAvailable() || typeof window === "undefined") {
    return false;
  }
  return window.localStorage.getItem(DEMO_MODE_STORAGE_KEY) === "true";
};

const subscribeToDemoMode = (onStoreChange: () => void) => {
  ...
  return () => {
    ...
    window.removeEventListener(DEMO_MODE_CHANGE_EVENT, handleChange);
  };
};
