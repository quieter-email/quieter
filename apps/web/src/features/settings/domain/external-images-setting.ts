...
const subscribeToExternalImagesEnabled = (onStoreChange: () => void) => {
  const handleStorage = (event: StorageEvent) => {
    if (event.key === EXTERNAL_IMAGES_STORAGE_KEY) {
      onStoreChange();
    }
  };
  const handleChange = () => {
  ...
  window.addEventListener(EXTERNAL_IMAGES_CHANGE_EVENT, handleChange);

  return () => {
    window.removeEventListener("storage", handleStorage);
    window.removeEventListener(EXTERNAL_IMAGES_CHANGE_EVENT, handleChange);
  };
};
