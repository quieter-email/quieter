...
export const messageListHeaderControlVariants = cva("", {
  variants: {
    control: {
      ...
      toolbar:
        "h-full w-8 rounded-xl border border-border bg-control text-muted-fg shadow-xs hover:bg-control-hover hover:text-fg [&_svg]:size-3.5",
      trigger:
        "squircle inline-flex h-full w-8 shrink-0 items-center justify-center rounded-xl border border-border bg-control text-muted-fg shadow-xs transition-colors duration-150 ease-out hover:bg-control-hover hover:text-fg active:bg-control-active active:text-fg disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-3.5 [&_svg]:shrink-0",
    },
  },
});
