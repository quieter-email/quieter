...
`build:runtime --service <registered-worker>` uses the same clean-source checks, isolated build environment, private source-map retention, and completion manifest as `build:web`. Background Workers compile through Wrangler's supported `deploy --dry-run --outdir` path, without uploading code. Each artifact records its service, and downloaded-build verification and release planning reject a service mismatch. Historical web artifacts without an explicit service remain identified as web builds. CI derives its Worker matrix from the runtime registry, limits concurrent builds to three, and retains one artifact per service. Temporary build configuration and compiler output are removed before completion so the retained directory contains only the verified release files. See [Wrangler build output](https://developers.cloudflare.com/workers/wrangler/commands/workers/).
...
Release preparation checks the complete candidate against the Git plan from the recorded healthy commit. Every affected runtime must be replaced by an artifact from the candidate commit, and unaffected runtimes must keep their recorded versions and contracts. Infrastructure, tooling, and unclassified changes require their separate reviewed transition. A changed healthy baseline invalidates preparation. Isolated provider drills can still use their synthetic source records; passing `--source` to `prepare` enables the real Git check for a proof stage too. Manual rollback selects an exact retained healthy release and does not require historical source to descend from the current commit.

Use `release candidate --release <new-release-id> --source <commit-sha> --file <upload-ids.json>` to construct that full candidate. The input is a JSON array of retained upload UUIDs. The command reconciles each upload against provider bytes, inherited bindings, and its unchanged baseline deployment, then checks the complete Git plan. Redirect its JSON output to the candidate file used by `prepare`. It preserves existing contract declarations; contract changes still require a reviewed candidate manifest. `release select-rollback` emits the most recent previous healthy release, or accepts `--release <retained-id>` to select an older one. Feed that exact output to `prepare --rollback`. Selection alone does not establish eligibility: preparation still verifies compatibility, retained artifacts, archives, quarantine, provider versions, and health configuration before activation.
...
`.github/workflows/release-recovery.yml` listens for release completion and reconciles every five minutes. `RUNTIME_RELEASE_RECOVERY_ENABLED` defaults off. Before enabling it, configure the `release-recovery` environment with a reviewed 40-character `RELEASE_CONTROLLER_SHA`, `RELEASE_STAGE`, `RELEASE_JOURNAL_BUCKET`, `CLOUDFLARE_ACCOUNT_ID`, `AWS_REGION`, and `RELEASE_RECOVERY_BINDINGS_PARAMETER`. Configure the intended `SENTRY_ORG`, `SENTRY_PROJECT`, and `SENTRY_URL` for retained receipt checks. Supply `RELEASE_RECOVERY_AWS_ROLE` with journal/version permissions and `ssm:GetParameter` on the exact recovery parameter ARN. It does not need access to SST's application secret store, a source-map upload token, the database, migrations, or an SST deployment.

`infra/release-operations.ts` derives separate runtime, recovery, and optional source-map `SecureString` parameters from SST Secrets. SST remains their source of truth. Runtime and recovery parameters contain their own Cloudflare token and the health-probe secret. The source-map parameter contains only its upload token. Standard parameters cap these small binding sets at 4 KiB. The operation runner fetches one exact ARN with decryption, checks its region, stage, purpose, and resource names, and passes the resulting linked bindings only to the selected repository CLI. It removes inherited SST links and old deployment tokens before launching that command. Grant `ssm:GetParameter` on individual ARNs, without parameter enumeration or path-wide access. See [AWS parameter retrieval](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_GetParameter.html).

For the isolated proof, set `QUIETER_RELEASE_OPERATION_BINDINGS=true` only after setting the stage's `ReleaseRuntimeCloudflareToken` and `ReleaseRecoveryCloudflareToken` SST Secrets. The existing `ReleaseProofToken` supplies the health credential. `QUIETER_RELEASE_SOURCE_MAP_UPLOAD=true` additionally uses `ReleaseSourceMapToken`. The proof stack exports the resulting parameter ARNs. This option defaults off and does not change the production stack. These new parameter bindings still need a live isolated verification after AWS sign-in is restored.

Set `QUIETER_RELEASE_BINDINGS_PARAMETER` to the intended exported ARN, along with the release bucket, stage, account, and destination metadata. Run `vp run @quieter/deployment#operations runtime release status`, `operations runtime archive <archive-options>`, `operations recovery reconcile`, or `operations source-maps upload <upload-options>`. The independent workflow uses the recovery command. CI build verification and receipt retention need no operational parameter or application secret.

### Background Worker health and preview access

The shared Worker entry wrapper reserves `GET /__release/health`. Enable it per runtime with `QUIETER_RUNTIME_HEALTH` containing `{"stage":"<stage>","publicHosts":["<canonical-host>"]}`, the existing linked `ReleaseProofToken` SST Secret and `App` binding, and a Cloudflare version metadata binding named `QUIETER_VERSION`. Use an empty host list for queue/scheduled Workers that expose no application HTTP routes. The stage must match the linked application. Enabled but incomplete bindings fail closed.

The endpoint requires the operation's bearer token and returns uncached `startup` evidence with the actual [provider version ID](https://developers.cloudflare.com/workers/runtime-apis/bindings/version-metadata/). Other requests may reach application handlers only on explicitly listed hosts. Preview hosts cannot call application routes, including with the health token. The wrapper preserves queue and scheduled handlers. Existing runtimes remain dormant until configured; the health route returns 404 while dormant.

`vp run @quieter/cloudflare#test:workers` exercises the wrapper in the native local runtime with test-only bindings, including authorization, incomplete configuration, preview denial, and queue dispatch. This adds startup evidence only. Runtime-specific dependency checks and post-activation queue/scheduled evidence remain required before cutover. No production health bindings were enabled by this change.

### Protected activation and rollback workflow

`Deploy Runtime Release` supports `promote` and `rollback` in one workflow. A successful trusted main build triggers promotion; manual promotion accepts that build run ID. It calculates all affected runtimes, downloads and rechecks their exact artifacts, retains CI/source-map evidence, archives browser files, and uploads inactive versions before constructing the full candidate. Rollback selects the previous healthy release by default, or a specified retained release ID, without downloading or rebuilding code. Both modes run pinned controller code under the protected production environment and share deployment concurrency with SST and independent recovery. Preparation records the attempt before activation. Failed activation/observation attempts compensation; cancellation or runner loss is handled by the independent recovery workflow after the writer ends.

The workflow is disabled unless both repository flags `RUNTIME_RELEASE_ENABLED` and `RUNTIME_RELEASE_RECOVERY_ENABLED` are true. The CLI's isolated-stage restriction remains in force until the ownership/recovery proof is complete. Infrastructure/tooling changes, unchanged sources, and plans requiring AWS runtimes stop before staging. Published Lambda versions and alias support remain pending. Failed staging can leave retained artifacts and inactive versions, but cannot change active traffic.

Configure `RELEASE_RUNTIME_AWS_ROLE`, the exact `RELEASE_RUNTIME_BINDINGS_PARAMETER` and `RELEASE_SOURCE_MAP_BINDINGS_PARAMETER` ARNs, journal/account/archive metadata, Sentry destination, and reviewed `RELEASE_CONTROLLER_SHA`. `RELEASE_PROBE_TARGETS` maps every service to its public health `url`, canonical Worker `previewUrl`, `checks`, and optional `criticalChecks`. For example, `{"web":{"url":"https://app.example.com/__release/health","previewUrl":"https://quieter-web.account.workers.dev/__release/health","checks":["startup","database"]}}`. Preparation derives both versioned preview URLs from the actual selected release records. A candidate that does not implement every configured check fails before activation. These settings and credentials are not configured in GitHub by this code change.
